...
function newApi(options) {
	...
	var initSyncRoute = require('./middleware/init-route.js');
	var syncHandler = options.handlers.sync.bind(instance);

	instance.middleware.use(initPassport(instance, passport, AnonymousStrategy, JwtStrategy));
	instance.middleware.use(initSyncUser);
	instance.middleware.use(initAnonymousUser(instance));
	instance.middleware.use(initBody);
	instance.middleware.use(initSyncRoute(instance, router));
	instance.middleware.use(syncHandler);

	// LISTENERS
	var useAsyncware = require('./listeners/use-asyncware.js');
	var useMiddleware = require('./listeners/use-middleware.js');

	instance.chainListener('connection', useAsyncware());
	instance.chainListener('request', useMiddleware());

	// REGISTRARS
	var viewRegistrar = require('./registrars/views.js');
	instance.addRegistrar({
		views: viewRegistrar(_, async)
	});
