...
function Api(options) {
	...
	options = _.defaultsDeep(options, {
		...
		registrars: {
			...
			views: require('./registrars/views')(_, async, passport)
		}
	});
	...
	api.express.use(initPassport(api, passport, AnonymousStrategy, JwtStrategy));
	api.express.use(initAnonymousUser(api));
	...
	api.chainListener('message', addMessageContext(_));
